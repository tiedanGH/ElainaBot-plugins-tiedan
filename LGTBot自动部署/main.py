...
两处**确定性事实注入**, 专治提示词管不住的小模型臆断: 原作标注在本地数 rule 文件里的
标注行数 (count_origin_marks); 输入回显在本地追踪字符串形参及其派生变量有没有进过发送
语句 (scan_echo_sends —— 以形参为种子、沿赋值传播到不动点, 覆盖「洗进局部或成员变量再
发出去」那种形态)。两者都只把**结论**写进系统提示词, 绝不把上传物原文挪进可信段落。
扫不到就明确告诉模型「已被排除, 不得以存在风险为由判定」; 扫到了只给可疑点清单, 判定
仍然交回模型。
