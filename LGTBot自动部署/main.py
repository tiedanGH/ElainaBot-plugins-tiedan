...
1.16.0 又按一个**真回显**补了四处: `oss << name` 流插入必须传播 (C++ 拼消息最常见的
写法, 之前漏了 —— 会把真回显扫成「已排除」再告诉模型不得判, 是这个扫描器最不能出的
错); 排除成员访问 (`c->name` 不是形参 name); 下标 / size() / find() 这类不携带原文
的用法不传播; 同名局部变量按声明点分实例 (每个函数各自的 oss 互不相干)。
